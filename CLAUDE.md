...
### 6. Auth is shell-owned; sessions ride in httpOnly cookies; MFEs read CSRF via a runtime contract
Login produces three Set-Cookies: `amp_access_token` (HttpOnly, path=/api, SameSite=Lax, 15m), `amp_refresh_token` (HttpOnly, path=/api/auth, SameSite=Strict, 7d), `amp_csrf_token` (JS-readable, SameSite=Strict). Access tokens never touch JS — the browser, not the axios interceptor, carries them. The shell writes `window.__AMP_PLATFORM__ = { user, csrfToken, logout }` when `AuthProvider` mounts. Each MFE's axios instance sets `withCredentials: true` and, for unsafe methods (POST/PUT/PATCH/DELETE), reads `csrfToken` off the SDK and echoes it as `X-CSRF-Token` (double-submit cookie). On a 401 the MFE dispatches `CustomEvent("amp:auth-expired")`; the shell tries a silent `/api/auth/refresh` before falling back to logout + redirect. Refresh rotations use reuse-detection — replaying a rotated refresh token wipes the whole session family. The Java tier hashes passwords with argon2id and runs rate limits (10/min on login, 30/min on refresh) via servlet filters. **Never** import auth code across packages — duplicate the `PlatformSdk` interface inline in each MFE. Protected route gating lives only in the shell's `<ProtectedRoute>` wrapper.
...
- The BFF does **not** validate tokens. It forwards the `Cookie` header verbatim to the monolith and lets the monolith reject invalid sessions — the browser's `amp_access_token` cookie rides /api/reporting/* automatically thanks to `withCredentials: true` on the MFE's axios. A `@RestControllerAdvice` in the BFF preserves upstream status codes so that a 401 from the monolith surfaces as a 401 to the MFE — which is what the `amp:auth-expired` flow (#6) expects.
...
- No real IdP. The Java `AuthProcess` reads hardcoded demo users from `db_migrations/MIGRATIONS/V20260422_00001_03__insert_demo_users.sql`. Passwords are seeded as plaintext so the migration stays human-readable, but the `AgentSystemStarterListener` calls `ensureArgon2Hashes()` on first boot and rehashes them in-place (argon2id) before the first request. Sessions are already httpOnly-cookie-based with refresh rotation + reuse detection + CSRF + rate limits — this is production-shape auth modulo the actual identity provider; swap for OIDC when a real IdP exists. The `/api/auth/demo-credentials` endpoint is demo-only — the seeded plaintext is surfaced from a hardcoded map in `AuthProcess` (the DB column is argon2-only and can't be reversed) and must not ship to production.
