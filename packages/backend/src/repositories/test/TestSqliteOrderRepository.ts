...
export class TestSqliteOrderRepository implements IOrderRepository {
  findAll(): Order[] {
    const stmt = testDb.prepare('SELECT * FROM orders ORDER BY created_at DESC');
  ...
  findById(id: string): Order | undefined {
    const stmt = testDb.prepare('SELECT * FROM orders WHERE id = ?');
  ...
  create(data: CreateOrderRequest): Order {
    ...
    const productStmt = testDb.prepare('SELECT price FROM products WHERE id = ?');
  ...
  delete(id: string): boolean {
    const stmt = testDb.prepare('DELETE FROM orders WHERE id = ?');
